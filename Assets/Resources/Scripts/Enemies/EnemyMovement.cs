...
public class EnemyMovement : MonoBehaviour
{
    ...
    public Enemy self;
    public Entity CurrentTarget { get { if (Targets.Count != 0) return Targets[0]; else return null; } }
    ...
    void FollowPlayer()
    {
        ...
        {
            if (!(CurrentTarget.transform.position.x - xOffSet <= transform.position.x && transform.position.x <= CurrentTarget.transform.position.x + xOffSet))
            {
                inXRange = false;
            ...
            if (!(CurrentTarget.transform.position.y - yOffSet <= transform.position.y && transform.position.y <= CurrentTarget.transform.position.y + yOffSet))
            {
                inYRange = false;
                if (CurrentTarget.transform.position.y > self.transform.position.y)
                {
